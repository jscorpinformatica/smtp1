...
if (preg_match('/iqdirect/i', $pasta_usuario_inbox))  $DB_PRINCIPAL = $DB_IQDIRECT;
...
{	   
	...
	if (is_file($file))
	{
		...
		$source = '';
		$procura23      = "TIRA-TEIMA";
		$comando      = "grep '$procura23' ".$file;	
		if( preg_match("/$procura23/",@shell_exec($comando)))
		{
			$comando = "cat $file | tr -d '=\n'";
			$out = shell_exec($comando);
			preg_match("/por favor, <a href3D\"(.*)\" target3D_blank><b>clique/",$out,$match);
			$url = $match[1];
			preg_match_all("/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}\b/i",$out,$match);
			$result = array_map('trim', $match[0]);
			$result = array_unique($result);
			$result = array_values($result);
			foreach($result as $email3)
			{	
				if(preg_match("/iqmail/",$email3))
				{
					$eqmail = $email3;$source="IQMAIL";
				}
				if(preg_match("/uol/",$email3))
				{
					$uol = $email3;
				}
				if(preg_match("/iqdirect/",$email3))
				{
					$eqmail = $email3;$source="IQDIRECT";
				}
			}
		...
		*/
	    ...
	    $vet_log_id = explode("\n",$message_id);
	    @$log_id     = $vet_log_id[1];
		$uol = "altivo@2atecnologia.com.br";
		$query = "SELECT table_name, create_time FROM information_schema.TABLES WHERE table_schema = 'iqmail_smtp' ORDER BY CREATE_TIME DESC LIMIT 10;";
		$DB_PRINCIPAL->query($query);
		$achei = false;
		while($DB_PRINCIPAL->next_record())
		{
			$DB2_PRINCIPAL = $DB_PRINCIPAL;
			$nome_tabela = $DB_PRINCIPAL->f("table_name");
			$query = "SELECT count(0), ivn_newsletter_id, envio2_id FROM iqmail_smtp.$nome_tabela WHERE email='$uol' AND status='E' GROUP BY ivn_newsletter_id";
			$DB2_PRINCIPAL->query($query);
			$DB2_PRINCIPAL->next_record();
			$resultado = $DB2_PRINCIPAL->f(0);
			$id_newsletter = $DB2_PRINCIPAL->f("ivn_newsletter_id");
			if($resultado >= 1)
			{
				/*Atualiza a tabela com o link da UOL*/
				echo $query."\n";
				echo "Newsletter ID = $id_newsletter \n \n"; 
				$bounce = 8;
				$query = "UPDATE iqmail_smtp.$nome_tabela SET bounce_status='$bounce', motivo_bounce='$url', numero_testes=numero_testes +1 WHERE ivn_newsletter_id='$id_newsletter' AND email='$uol'";
				echo $query."\n";
				break;
			}
			continue;
	    ...
	    if (!f_busca_dados_envio($DB_PRINCIPAL, $log_id, $log_news, $log_envioid)){
		   ...
		   if (!f_busca_dados_envio($DB_PRINCIPAL, $log_id, $log_news, $log_envioid)){
		     ...
		     #$comando = "mv $file $pasta_usuario_inbox/avaliar/";#COM
		     #@shell_exec($comando);
		      continue;
		   }   
	    }   
